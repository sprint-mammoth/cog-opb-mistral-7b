...
example_prompt = "What are the main challenges to support a long context for LLM?"
PROMPT_TEMPLATE = [
   "<|prompter|>{prompt}?</s><|assistant|>",
]
MISTRAL_OPENORCA_PROMPT_TEMPLATE = """\
<|im_start|>system
...
def vllm_generate_iterator(
    self, prompt: str, /, *, echo: bool = False, stop: str  = None, stop_token_ids: List[int] = None, sampling_params=None, **attrs: Any
...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        max_new_tokens: int = Input(
            description="The maximum number of tokens the model should generate as output. max=16K tokens.",
